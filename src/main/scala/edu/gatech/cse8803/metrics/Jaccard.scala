...
object Jaccard
{    
   def jaccardSimilarityOneVsAll(graph: Graph[VertexProperty, EdgeProperty], patientID: String, casecontrol: List[String]): List[String] = 
   {
        ...
        var neighborgraph =  graph
          ...
          .filter{case (idx, neighbors) => neighbors.size > 0 &&  casecontrol.contains((-1 * idx).toString) && idx < 0 && !neighbors.exists{case(_, p) => p.isInstanceOf[PatientProperty]}}
